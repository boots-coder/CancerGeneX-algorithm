...
class TransformerWrapper(ModelWrapper):
    def __init__(self, name, kwargs):
        ...
        # 动态调整模型参数
        nhead = max(4, min(8, input_size // 64))
        num_layers = max(2, min(4, input_size // 128))  # 减少编码层
        dim_feedforward = max(128, min(256, input_size * 2))
    ...
    class AttentionPooling(nn.Module):
        ...
        def forward(self, x):
            # x shape: [batch_size, seq_len, dim]
            attention_weights = F.softmax(self.attention(x), dim=1)
            attention_weights = self.dropout(attention_weights)  # Dropout 防止过拟合
            attended = torch.sum(attention_weights * x, dim=1)
            return attended
    ...
    class TransformerNet(nn.Module):
        def __init__(self, InputSize, ClassNum, nhead=4, num_layers=2, dim_feedforward=128):
            ...
            # 确保 model_dim 是 head 数量的倍数
            self.model_dim = max(64, min(128, InputSize // 4))
            ...
            # 注意力池化
            self.attention_pooling = TransformerWrapper.AttentionPooling(self.model_dim, dropout_rate=0.1)
        ...
        def forward(self, x):
            ...
            if len(x.shape) == 2:
                x = x.unsqueeze(1)  # [batch_size, 1, InputSize]
